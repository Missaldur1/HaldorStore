  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const rel = all
          .filter(
            (x) => x.category.slug === p.category.slug && x.slug !== p.slug
          )
          .slice(0, 6)
        setRelated(rel)
      } catch {
...
function Breadcrumb({ product }: { product: Product }) {
  return (
    <nav aria-label="breadcrumbs" className="text-xs text-stone-400">
      ...
      <Link
        to={`/catalog?category=${encodeURIComponent(product.category.slug)}`}
  ...
  return (
    <div className="rounded-2xl border border-stone-700/50 bg-gradient-to-b from-stone-900/70 to-slate-900/70 p-5 sm:p-6">
      ...
      <div className="mt-2 flex items-center gap-3">
        ...
        <span className="ml-auto text-xs rounded-full border px-2 py-0.5 border-stone-700/60 bg-stone-900/50">
          {stockLabel}
        </span>
      </div>

      <div className="mt-4 text-3xl font-extrabold text-amber-300">
        {priceText}
      </div>

      <p className="mt-3 text-sm text-stone-300/90">
        {product.description ??
          "Diseño nórdico moderno, materiales duraderos y terminaciones de calidad."}
      </p>
  ...
  return (
    <div className="flex">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          key={i}
          className={`size-4 ${i < r ? "text-amber-400" : "text-stone-500"}`}
...
function DetailsTabs({ product }: { product: Product }) {
  ...
  return (
    <div className="rounded-2xl border border-stone-700/50 overflow-hidden">
      <div className="flex gap-1 border-b border-stone-700/60 bg-stone-900/70 px-3 py-2">
        ...
        <TabBtn active={tab === "envio"} onClick={() => setTab("envio")}>
          Envío y cambios
        </TabBtn>
      </div>
      ...
      <div className="bg-stone-900/60 p-4">
        {tab === "desc" && (
          <p className="text-sm text-stone-300/90 max-w-prose">
            {product.long_description ??
              "Inspirado en el norte, fabricado con materiales duraderos."}
          </p>
        )}
        ...
        {tab === "envio" && (
          <ul className="text-sm text-stone-300/90 list-disc pl-5 space-y-1">
            <li>Despachos entre 24 y 72 hrs.</li>
            <li>Cambios 30 días; devoluciones 10 días.</li>
...
function RelatedGrid({ items }: { items: Product[] }) {
  return (
    <section>
      <h2 className="text-xl sm:text-2xl font-bold mb-3">Relacionados</h2>
...
function RelatedCard({ product }: { product: Product }) {
  ...
  return (
    <article className="group rounded-xl border border-stone-700/50 bg-stone-900/70 overflow-hidden hover:border-amber-500/40">
